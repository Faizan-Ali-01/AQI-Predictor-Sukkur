...
DATA_DIR = 'data'
MODEL_DIR = 'models'
CURRENT_CSV = os.path.join(DATA_DIR, 'aqi_sukkur.csv')
...
LAT = float(os.getenv('LAT', '31.5497'))
LON = float(os.getenv('LON', '74.3436'))
OPENWEATHER_KEY = os.getenv('OPENWEATHER_KEY')

# prefer city-specific CSV if present
CURRENT_CSV = os.path.join(DATA_DIR, f"aqi_{CITY.lower()}.csv")
...
# === PAGE CONFIG ===
st.set_page_config(
    page_title=f"{CITY} AQI Predictor",
    page_icon="🌍",
    layout="wide",
...
<style>
    ...
    .stApp {
        background: linear-gradient(135deg, #f6fbff 0%, #eef6fa 100%);
        color: #073b4c;
        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial;
    }

    /* Main header styling tweaks */
    .stTitle, .stHeader {
    ...
    .aqi-card {
        ...
        box-shadow: 0 6px 20px rgba(8,61,68,0.08);
        border-radius: 18px;
        padding: 26px;
        text-align: center;
    ...
    /* Small metric tiles */
    .metric-card {
        background: rgba(6,54,63,0.03);
        border-radius: 12px;
        padding: 12px 10px;
        text-align: center;
        border-left: 4px solid rgba(3,109,120,0.18);
        margin: 6px 6px;
        min-width: 0;
        word-break: keep-all;
    }
    ...
    .section-title {
        ...
        border-bottom: 2px solid rgba(5,102,141,0.08);
        padding-bottom: 6px;
    }

    /* Plotly chart backgrounds */
    .js-plotly-plot .plotly .bg { fill: transparent !important; }
    .js-plotly-plot .plotly paper { background: transparent !important; }

    /* Buttons */
    div.stButton > button {
        background: linear-gradient(90deg,#05668d,#028090) !important;
        color: #fff !important;
        border: none !important;
        padding: 8px 14px !important;
        border-radius: 10px !important;
        box-shadow: 0 6px 18px rgba(2,128,144,0.12) !important;
    }
    div.stButton > button:hover { filter: brightness(1.03); }

    /* Sidebar tweaks (compatible selectors) */
    section[aria-label="Sidebar"] {
        background: linear-gradient(180deg,#ffffffee,#f8fbfd) !important;
        border-left: 1px solid rgba(8,61,68,0.04);
    ...
    /* Responsive tweaks for metric tiles */
...
def show_aqi_card(aqi_value, status, color):
    st.markdown(f"""
    <div class="aqi-card">
        ...
        <div style="font-size:20px; color:{color}; margin-top:10px; font-weight:600;">
            {status}
        </div>
    ...
    """, unsafe_allow_html=True)
def show_forecast_tile(day_label, aqi, status, color):
    st.markdown(f"""
    <div class="metric-card">
        <div style="color:#aaa; font-size:13px;">{day_label}</div>
        <div style="font-size:36px; font-weight:800; color:{color};">{aqi}</div>
...
ow_api_key = OPENWEATHER_KEY
ow_url = f"http://api.openweathermap.org/data/2.5/air_pollution?lat={LAT}&lon={LON}&appid={ow_api_key}"
api_success = False
...
if api_success:
    ...
    show_aqi_card(aqi, aqi_status, aqi_color)
    if aqi > 150:
    ...
    for col, label, val in zip(cols, pollutant_labels, pollutant_vals):
        col.markdown(f"""
<div class="metric-card">
    <div style="color:#a78bfa; font-size:13px; font-weight:600;">{label}</div>
...
<div class="metric-card">
    <div style="color:#a78bfa; font-size:13px; font-weight:600;">{label}</div>
    <div style="font-size:28px; font-weight:800; color:white;">{f"{val:.2f}" if pd.notnull(val) else "N/A"}</div>
</div>
""", unsafe_allow_html=True)
    except Exception as e:
...
try:
    ...
    fig = px.bar(model_df, x='Model', y='RMSE', color='Model',
                 color_discrete_sequence=['#7c3aed', '#a78bfa', '#4f46e5'],
    ...
    if api_ok and forecast_data is not None:
        ...
        for d in range(3):
            ...
            if len(day_slice) > 0:
                ...
                status, color = get_aqi_status(int(day_val))
                with cols[d]:
                    show_forecast_tile(f"Day {d+1}", int(day_val), status, color)
        ...
        ow_url = f"http://api.openweathermap.org/data/2.5/forecast?lat={LAT}&lon={LON}&appid={ow_api_key}"
        ...
        air_url = f"http://api.openweathermap.org/data/2.5/air_pollution?lat={LAT}&lon={LON}&appid={ow_api_key}"
...
try:
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.write("SHAP (SHapley Additive exPlanations) shows how much each feature contributes to AQI predictions. PM2.5 is the dominant factor in Sukkur's AQI.")
    if len(shap_df) >= 3:
...
try:
    ...
    fig.update_layout(         # ← yeh naya add karo
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(255,255,255,0.05)',
            font_color='white',
            title_font_color='#a78bfa',
...
st.markdown("---")
st.caption(f"© 2026 {CITY} AQI Predictor | Built with Streamlit, Plotly, SHAP, and Python")