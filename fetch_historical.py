...
def fetch_openaq_data(api_key, city="Sukkur", country="PK", parameters=None, days=365):
    if parameters is None:
        parameters = ["pm25", "pm10", "no2", "so2", "o3", "co"]
    headers = {"X-API-Key": api_key}
    base_url = "https://api.openaq.org/v3/"
    # Step 1: Get Sukkur location IDs
    try:
        ...
        if not location_ids:
            print("No location IDs found for Sukkur in OpenAQ.")
            return pd.DataFrame()
...
# Fetch current weather for Sukkur using free endpoint
...
def main():
    ...
    print("Fetching current weather for Sukkur...")
